...
export class Conf extends React.Component<{}, {}> {
  ...
  private renderConference(
    streamProps: IStreamsRendererProps,
    controlProps: IMediaStreamControlRendererProps
      ...
      <div className="docs-conf">
        {localStream ? (
          <div className="docs-conf-local-stream">
            <Stream
              mirror={!localStream.isScreenSharing}
              stream={localStream.stream}
              muted={true}
              isRecording={localStream.isRecording}
            />
          </div>
        ) : null}
